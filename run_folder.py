...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run PDC_v3 on a folder of images.")
    parser.add_argument("--input-dir", required=True, help="Folder containing input images")
    parser.add_argument("--output-dir", default="outputs", help="Directory for JSON and annotated images")
...
def main() -> None:
    ...
    payload = pipeline.process_folder(
        args.input_dir,
        output_dir=args.output_dir,
        recursive=args.recursive,
        include_keypoints=args.include_keypoints,
    )
    print(f"[PDC] Done. Images: {len(payload.get('images', []))}; annotations: {len(payload.get('annotations', []))}")
    if payload.get("errors"):
        print(f"[PDC] Completed with {len(payload['errors'])} image error(s). See annotations.json.")
